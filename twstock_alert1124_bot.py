...
INTRADAY_STATE = {}
STOCK_NAMES    = {}
PREV_CLOSE     = {}   # 昨收價快取，從 STOCK_DAY_ALL 取得（比 MIS y 欄準確）
            ...
            if r.status_code == 200:
                ...
                if arr:
                    ...
                    if price is not None:
                        # 注意：不使用 MIS 的 y 欄位作為昨收，因為可能是參考價而非昨收
...
def analyze_stock(symbol, tw_time):
    ...
    prev  = get_prev_close(symbol)   # 只用 STOCK_DAY_ALL 的昨收，不用 MIS y 欄
    ...
    if text.startswith('/track'):
        ...
        for sym in symbols:
            ...
            if info and info.get('price') is not None:
                curr    = info['price']
                name    = info.get('name') or name   # 優先用 MIS 回傳的名稱
