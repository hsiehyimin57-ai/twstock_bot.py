...
def generate_post_market_msg():
    ...
    def fmt_ranking(stocks, is_buy):
        ...
        for item in stocks:
            ...
            name = pad_str(item['name'][:4], 6)
            lots = abs(item['net']) // 1000
            rows.append(f"{code}{name}{lots:>7,}")
        return ["```\n" + "\n".join(rows) + "\n```"]
...
def analyze_stock(symbol, tw_time):
    ...
    分析單一股票，回傳 dict:
    {
      'open':  (label, line) or None,
      'timed': (label, line) or None,
      'high':  [(label, line), ...],
      'close': (label, line) or None,
    }
    """
    ...
    result  = {'open': None, 'timed': None, 'high': [], 'close': None}
    ...
    if price > state['day_high'] > 0 and t >= '09:00':
        result['high'].append(make_stock_line(symbol, name, price, prev))
    ...
    while True:
        ...
        if TRACK_LIST and '09:00' <= time_str <= '13:35' and tw_time.weekday() < 5:
            ...
            for line in high_lines:
                send_table(CHAT_ID, f"🔥 【創新高 {t}】", [line])
