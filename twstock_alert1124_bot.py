...
def generate_post_market_msg():
    ...
    def fmt_ranking(stocks, is_buy):
        """回傳 Markdown code block 表格（用 ``` 包住）"""
        act_label = "買超張數" if is_buy else "賣超張數"
        rows = [f"{'代號':<8} {'名稱':<10} {act_label:>8}"]
        rows.append("-" * 30)
        for item in stocks:
            code = item['code']
            name = item['name'][:8]
            lots = abs(item['net']) // 1000
            rows.append(f"{code:<8} {name:<10} {lots:>8,}")
        return ["```\n" + "\n".join(rows) + "\n```"]
...
def fmt_stock_row(symbol, name, price, prev, label=""):
    """
    回傳等寬格式的單行，用 ``` 包住（中文字寬修正）：
    代號     名稱       股價       漲跌    漲跌幅
    2330     台積電  1780.00   -20.00   -1.11%
    """
    name_short = (name or '')[:6]
    code_col  = pad_str(symbol[:7], 8)
    name_col  = pad_str(name_short, 8)
...
def analyze_and_alert(symbol, tw_time):
    ...
    if t >= '09:00' and not state['reported_open'] and open_ and open_ > 0:
        alerts.append(f"🎯 【開盤 {t}】")
        alerts.append(fmt_stock_row(symbol, name, open_, prev))
...
def handle(update):
    ...
    if text.startswith('/track'):
        ...
        # 個股表格（每 10 檔一組）
        header_row = pad_str("代號", 8) + pad_str("名稱", 8) + f"{'股價':>8} {'漲跌':>8} {'漲跌幅':>8}"
        sep_row    = "─" * 50
        ...
        for sym in symbols:
            ...
            if info and info.get('price') is not None:
                ...
                code_col  = pad_str(sym[:7], 8)
                name_col  = pad_str((name or '')[:6], 8)
                ...
                rows.append(pad_str(sym, 8) + pad_str("查詢失敗", 8))
            time.sleep(0.2)
            # 每 10 筆送出一則
            if len(rows) - 2 >= 10:
                send_md(chat_id, "```\n" + "\n".join(rows) + "\n```")
