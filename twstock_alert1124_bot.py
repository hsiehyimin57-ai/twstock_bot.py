...
def fetch_bulk_closing_prices():
    prices = {}
    try:
        r = requests.get('https://openapi.twse.com.tw/v1/exchangeReport/STOCK_DAY_ALL', headers=HEADERS_WEB, timeout=15)
        if r.status_code == 200:
            for item in r.json():
        ...
        pass
    try:
        r = requests.get('https://www.tpex.org.tw/openapi/v1/t187ap03_L', headers=HEADERS_WEB, timeout=15)
        if r.status_code == 200:
            for item in r.json():
...
# ────────────────────────────────────────────────
# TWSE MIS 即時報價
# 注意：不使用 MIS 的 y 欄位作為昨收（可能是參考價，不準）
# 昨收只從 STOCK_DAY_ALL 取得
# ────────────────────────────────────────────────
...
def generate_post_market_msg():
    ...
    # 3. 外資排行
    data_f, _, err_f = fetch_twse_rwd("https://www.twse.com.tw/rwd/zh/fund/TWT38U?date={}&response=json", trade_date)
    if data_f:
        try:
            s = parse_ranking(data_f, '買賣超')
            if s:
            ...
            msgs.append(f"⚠️ 外資排行錯誤: {e}")
    else:
        msgs.append(f"⚠️ 外資排行: 失敗 ({err_f})")

    msgs.append("-------------------------")

    # 4. 投信排行
    data_t, _, err_t = fetch_twse_rwd("https://www.twse.com.tw/rwd/zh/fund/TWT44U?date={}&response=json", trade_date)
    if data_t:
        try:
            s = parse_ranking(data_t, '買賣超')
            if s:
            ...
            msgs.append(f"⚠️ 投信排行錯誤: {e}")
    else:
        msgs.append(f"⚠️ 投信排行: 失敗 ({err_t})")

    msgs.append("-------------------------")

    # 5. 自營商排行
    data_d, _, err_d = fetch_twse_rwd("https://www.twse.com.tw/rwd/zh/fund/TWT43U?date={}&response=json", trade_date)
    if data_d:
        try:
            s = parse_ranking(data_d, '買賣超')
            if s:
            ...
            msgs.append(f"⚠️ 自營商排行錯誤: {e}")
    else:
        msgs.append(f"⚠️ 自營商排行: 失敗 ({err_d})")
...
def analyze_stock(symbol, tw_time):
    ...
    prev  = get_prev_close(symbol)   # 只用 STOCK_DAY_ALL 昨收，不用 MIS y 欄
...
def handle(update):
    ...
    if text.startswith('/track'):
        ...
        if len(parts) > 1:
            added = []
            for s in parts[1:]:
                if s not in TRACK_LIST and len(TRACK_LIST) < 20:
                    TRACK_LIST.append(s)
                    added.append(s)
            if added:
                send(chat_id, f"✅ 已新增：{', '.join(added)}\n📋 目前清單：{', '.join(TRACK_LIST)}")
            else:
                send(chat_id, "⚠️ 未新增（已在清單中或已達20檔上限）")
        else:
            send(chat_id, "請輸入要新增的股票代號，例如：/add 2330 2603")
        ...
        if len(parts) > 1:
            ...
            for s in parts[1:]:
                if s in TRACK_LIST:
                    ...
                    removed.append(s)
            if removed:
                send(chat_id, f"✅ 已移除：{', '.join(removed)}\n📋 目前清單：{', '.join(TRACK_LIST) if TRACK_LIST else '（空）'}")
            else:
                send(chat_id, "⚠️ 指定代號不在清單中")
        else:
            send(chat_id, "請輸入要移除的股票代號，例如：/remove 2330")

    elif text.startswith('/list'):
        if TRACK_LIST:
            send(chat_id, f"📋 追蹤清單（{len(TRACK_LIST)}/20）：{', '.join(TRACK_LIST)}")
        else:
