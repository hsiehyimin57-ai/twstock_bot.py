...
GITHUB_REPO  = os.environ.get('GITHUB_REPO', '')   # 格式：帳號/repo名稱
...
# GitHub 追蹤清單持久化
# ────────────────────────────────────────────────
def load_track_list_from_github():
    """bot 啟動時從 GitHub 讀取 track_list.json"""
    global TRACK_LIST
    content_str = github_get_file('track_list.json')
    if content_str:
        try:
            data = json.loads(content_str)
            if isinstance(data, list) and data:
                TRACK_LIST = data
                logging.info(f"GitHub 追蹤清單載入 {len(TRACK_LIST)} 檔: {', '.join(TRACK_LIST)}")
        except Exception as e:
            logging.warning(f"GitHub 追蹤清單載入失敗: {e}")

def save_track_list_to_github():
    """清單變更後寫入 GitHub track_list.json"""
    if not GITHUB_TOKEN or not GITHUB_REPO:
        return
    import base64
    api_url = f"https://api.github.com/repos/{GITHUB_REPO}/contents/track_list.json"
    headers = {
        'Authorization': f'token {GITHUB_TOKEN}',
        'Accept': 'application/vnd.github.v3+json'
    }
    content_str = json.dumps(TRACK_LIST, ensure_ascii=False)
    ...
    try:
        body = {
            'message': f'更新追蹤清單 ({len(TRACK_LIST)} 檔)',
            'content': content_b64,
        }
        if sha:
            body['sha'] = sha
        r = requests.put(api_url, headers=headers, json=body, timeout=10)
        if r.status_code in (200, 201):
            logging.info(f"GitHub 追蹤清單已儲存 ({len(TRACK_LIST)} 檔)")
        else:
            logging.warning(f"GitHub 追蹤清單儲存失敗: HTTP {r.status_code}")
    except Exception as e:
        logging.warning(f"GitHub 追蹤清單儲存失敗: {e}")

# ────────────────────────────────────────────────
# 昨收價：優先 GitHub，fallback API
# ────────────────────────────────────────────────
def github_get_file(filename):
    """用 GitHub API 讀取檔案內容（不依賴 raw URL，支援特殊字元 repo 名稱）"""
    if not GITHUB_TOKEN or not GITHUB_REPO:
        return None
    import base64
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            logging.warning(f"GitHub API 讀取 {filename} 失敗: HTTP {r.status_code}")
    except Exception as e:
        logging.error(f"GitHub API 讀取錯誤: {e}")
    return None

def load_prev_close_from_github():
    """從 GitHub 用 API 載入 prev_close.json"""
    global PREV_CLOSE
    content_str = github_get_file('prev_close.json')
    if content_str:
        try:
            data = json.loads(content_str)
            if data:
                PREV_CLOSE.update(data)
                logging.info(f"GitHub API 昨收載入 {len(data)} 筆")
                return True
        except Exception as e:
            logging.error(f"GitHub 昨收解析錯誤: {e}")
    return False
    try:
...
def load_prev_close_from_api():
    ...
    try:
        tpex_ok = False
        for tpex_url, code_key, price_key in [
            ('https://www.tpex.org.tw/openapi/v1/t187ap03_L', 'SecuritiesCompanyCode', 'ClosingPrice'),
            ('https://www.tpex.org.tw/openapi/v1/tpex_mainboard_daily_close_quotes', 'SecuritiesCompanyCode', 'ClosingPrice'),
        ]:
            try:
                r = requests.get(tpex_url, headers=HEADERS_WEB, timeout=15, verify=False)
                if r.status_code == 200 and r.text.strip():
                    for item in r.json():
                        code = str(item.get(code_key, '') or item.get('Code', ''))
                        v = (item.get(price_key, '') or item.get('Close', '')).replace(',', '')
                        if code and v:
                            try:
                                PREV_CLOSE[code] = float(v)
                                count += 1
                                ...
                                pass
                    tpex_ok = True
                    logging.info(f"TPEx 昨收 ({tpex_url.split('/')[-1]}): {count} 筆")
                    break
            except Exception as e:
                logging.warning(f"TPEx 昨收端點失敗 ({tpex_url.split('/')[-1]}): {e}")
        if not tpex_ok:
            logging.error("TPEx 昨收: 所有端點均失敗")
    except Exception as e:
...
def update_stock_names():
    ...
    try:
        # 嘗試多個 TPEx 端點
        tpex_ok = False
        for tpex_url, code_key, name_key in [
            ('https://www.tpex.org.tw/openapi/v1/t187ap03_L', 'SecuritiesCompanyCode', 'CompanyName'),
            ...
            try:
                ...
                pass
        if not tpex_ok:
            logging.warning("TPEx 名稱更新失敗，所有端點均無效")
    except Exception as e:
            ...
            try:
                ...
                if r.status_code == 200 and r.text.strip():
                    for item in r.json():
                        ...
                        price = (item.get(price_key, '') or item.get('Close', ''))
                        if code:
                            prices[code] = price
                    break
            except Exception:
...
def handle(update):
    ...
    if text.startswith('/track'):
        ...
        if len(parts) > 1:
            ...
            threading.Thread(target=save_track_list_to_github, daemon=True).start()
...
if __name__ == '__main__':
    ...
    update_stock_names()
    update_prev_close()
    load_track_list_from_github()   # 恢復上次的追蹤清單
