...
def load_prev_close_from_github():
    ...
    try:
        ...
        if r.status_code == 200:
            PREV_CLOSE.update(r.json())
            logging.info(f"GitHub 昨收載入 {len(PREV_CLOSE)} 筆")
            return True
...
def make_stock_line(symbol, name, price, prev):
    """
    等寬格式：代號(7) 名稱前4字(8) 股價(8) 漲跌(8) 漲跌幅(7)
    範例：
    2330   台積電      1860   +60  +3.33%
    2303   聯電        58.30  +4.70 +8.77%
    """
    code_s  = pad_str(symbol[:6], 7)
    name_s  = pad_str((name or '')[:4], 8)
