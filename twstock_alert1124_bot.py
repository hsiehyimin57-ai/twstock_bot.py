...
# GitHub raw URL，格式：https://raw.githubusercontent.com/你的帳號/你的repo/main/prev_close.json
...
INTRADAY_STATE = {}
STOCK_NAMES    = {}
PREV_CLOSE     = {}   # 昨收價，從 GitHub prev_close.json 載入
...
# ────────────────────────────────────────────────
# 昨收價：優先從 GitHub 載入，fallback 到 TWSE OpenAPI
# ────────────────────────────────────────────────
def load_prev_close_from_github():
    """從 GitHub raw URL 載入 prev_close.json"""
    global PREV_CLOSE
    if not GITHUB_PREV_CLOSE_URL:
        logging.warning("GITHUB_PREV_CLOSE_URL 未設定，跳過 GitHub 載入")
        return False
    try:
        r = requests.get(GITHUB_PREV_CLOSE_URL, timeout=15)
        if r.status_code == 200:
            ...
            logging.warning(f"GitHub 昨收載入失敗: HTTP {r.status_code}")
    except Exception as e:
        logging.error(f"GitHub 昨收載入錯誤: {e}")
    return False

def load_prev_close_from_api():
    """從 TWSE/TPEx OpenAPI 載入昨收（fallback）"""
    global PREV_CLOSE
    count = 0
    try:
        r = requests.get(
        ...
        logging.error(f"TWSE API 昨收失敗: {e}")
    try:
        r = requests.get(
        ...
        logging.error(f"TPEx API 昨收失敗: {e}")
...
def make_stock_line(symbol, name, price, prev):
    """
    等寬格式一行：代號(7) 名稱前3字(6) 股價(8) 漲跌(8) 漲跌幅(7)
    例：2330   台積電  1810.00  -20.00  -1.09%
    """
    code_s  = pad_str(symbol[:6], 7)
    name_s  = pad_str((name or '')[:3], 6)
    price_s = f"{fmt_price(price):>8}"
    if prev and prev > 0:
        chg   = price - prev
        pct   = chg / prev * 100
        chg_s = f"{chg:>+8.2f}"
